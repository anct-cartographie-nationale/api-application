...
import { fromLieuxInclusionNumeriqueStorage, FingerprintTransfer } from './fingerprint.transfer';
...
describe('fingerprint transfer', (): void => {
  it('should not map lieux inclusion numerique storage to fingerprints to transfer when sourceId is missing', (): void => {
    ...
    const fingerprints: FingerprintTransfer[] = fromLieuxInclusionNumeriqueStorage(lieux);

    expect(fingerprints).toStrictEqual<FingerprintTransfer[]>([]);
  });
